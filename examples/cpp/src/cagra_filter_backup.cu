...
void compute_recall_filtered(raft::device_resources const& dev_resources, 
                    std::vector<raft::device_matrix<float, int64_t>> cat_data,
                    raft::device_matrix_view<const float, int64_t> queries,
                    raft::device_vector_view<const int32_t, int64_t> query_cats,
                    raft::device_matrix_view<uint32_t> neighbors,
                    int topk) {

  raft::handle_t handle;

printf("starting compute_recall...\n");
  int dim = queries.extent(1);
  int total_queries = queries.extent(0);
  cudaStream_t stream = raft::resource::get_cuda_stream(dev_resources);

  auto groundtruth = raft::make_host_matrix<uint32_t>(handle, total_queries, topk);

  auto h_queries = raft::make_host_matrix<float, int64_t>(handle, total_queries, dim);
  raft::copy(h_queries.data_handle(), queries.data_handle(), queries.size(), stream);
  auto h_query_cats = raft::make_host_vector<int32_t, int64_t>(handle, total_queries);
  raft::copy(h_query_cats.data_handle(), query_cats.data_handle(), queries.size(), stream);

printf("A\n");
  

  for(int cat=0; cat<cat_data.size(); cat++) {
    auto data_view = cat_data[cat].view();

    int n_cat_queries = 0;
    for(int i=0; i<h_query_cats.size(); i++) {
      if(h_query_cats(i) == cat) n_cat_queries++;
    }
    raft::resource::sync_stream(dev_resources);

printf("cat:%d, n_cat_queries:%d, dim:%d, allocating...\n", cat, n_cat_queries, dim);

    auto h_temp_queries = raft::make_host_matrix<float, int64_t>(handle, n_cat_queries, dim);

printf("alloc done, copying data...\n");

    int write_idx=0;
    for(int i=0; i<h_query_cats.size(); i++) {
      if(h_query_cats(i) == cat) {
        for(int j=0; j<dim; j++) {
          h_temp_queries(write_idx,j) = h_queries(i,j);
          write_idx++;
        }
      }
    }

printf("B - num queries:%d\n", n_cat_queries);
    auto temp_queries = raft::make_device_matrix<float, int64_t>(dev_resources, n_cat_queries, dim);
    raft::copy(temp_queries.data_handle(), h_temp_queries.data_handle(), h_temp_queries.size(), stream);
    
printf("C\n");
    // Run BF to get neighbors
    auto bf_index = cuvs::neighbors::brute_force::build(dev_resources, data_view);
    auto bf_neighbors = raft::make_device_matrix<int64_t,int64_t>(dev_resources, n_cat_queries, topk);
    auto bf_distances = raft::make_device_matrix<float,int64_t>(dev_resources, n_cat_queries, topk);
    ...
    auto h_cat_neighbors = raft::make_host_matrix<int64_t,int64_t>(handle, n_cat_queries, topk);
    raft::copy(h_cat_neighbors.data_handle(), bf_neighbors.data_handle(), bf_neighbors.size(), stream);

printf("D\n");
    // Copy result into total query list results
    int read_idx=0;
    for(int i=0; i<h_query_cats.size(); i++) {
      if(h_query_cats(i) == cat) {
        for(int j=0; j<topk; j++) {
          groundtruth(i,j) = h_cat_neighbors(read_idx,j);
        }
      }
    }
  }
printf("E\n");
  ...
  // Compare newly computed ground truth with neighbor list provided
  auto h_neighbors = raft::make_host_matrix<uint32_t>(handle, total_queries, topk);
  raft::copy(h_neighbors.data_handle(), neighbors.data_handle(), neighbors.size(), stream);
  
  printf("computing filtered recall...\n");
  int correct=0;
  for(int i=0; i<neighbors.extent(0); i++) {
    for(int j=0; j<topk; j++) {
      for(int k=0; k<topk; k++) {
        if(h_neighbors(i,j) == groundtruth(i,k)) {
          correct++;
          break;
        }
      }
  ...
  printf("Filtered recall - queries:%d, topk:%d, correct:%d, recall:%f\n", total_queries, topk, correct, (float)correct / (float)(total_queries*topk));
