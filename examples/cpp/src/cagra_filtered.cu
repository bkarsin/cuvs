...
void generate_categories(raft::device_resources const& dev_resources,
                      raft::device_matrix_view<int32_t> dataset,
		      ...
		      int num_categories, int density)
{
  ...
  raft::copy(dataset.data_handle(), labels.data_handle(), labels.size(), stream);
...
int main()
{
  ...
  auto data_cats = raft::make_device_matrix<int32_t,int64_t>(dev_resources, n_samples, density);
  auto query_cats = raft::make_device_vector<int32_t>(dev_resources, n_queries);

  generate_categories(dev_resources, data_cats.view(), query_cats.view(), n_categories, density);
